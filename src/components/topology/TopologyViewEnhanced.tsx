...
const NodeActionMenu: React.FC<{
  ...
  onOpenSftp: () => void;
}> = ({ node, x, y, onClose, onNavigateToSession, onCreateTerminal, onOpenSftp }) => {
  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-1.5 border-t border-zinc-700/50 bg-zinc-800/30">
        <div className="text-[10px] text-zinc-500 text-center">点击外部关闭</div>
      </div>
    </motion.div>
  );
};
...
export const TopologyViewEnhanced: React.FC<TopologyViewEnhancedProps> = ({ 
    ...
    return (
      <div className="flex items-center justify-center h-full text-zinc-500">
        <div className="text-center">
          ...
          <p className="text-sm font-mono tracking-widest uppercase">No Active Matrix</p>
          <p className="text-xs mt-2 opacity-50">Connect to servers to populate</p>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    <div className="absolute bottom-4 left-4 z-10 text-[10px] text-zinc-600 font-mono">
      Scroll to zoom · Drag to pan · Double-click node for menu
    </div>
