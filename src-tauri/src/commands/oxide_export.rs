...
use crate::oxide_file::{
    compute_checksum, decrypt_oxide_file, encrypt_oxide_file, EncryptedAuth, EncryptedConnection,
    EncryptedPayload, OxideMetadata,
};
            ...
            EncryptedAuth::Key {
                ...
                let passphrase_keychain_id = if let Some(pass) = passphrase {
                    ...
                    if let Err(e) = config_state.set_keychain_value(&kc_id, &pass) {
                        errors.push(format!(
                            "Failed to store passphrase for {}: {}",
                            enc_conn.name, e
                        ));
                        continue;
                    }
