...
use super::format::{OxideFile, OxideMetadata, EncryptedPayload, EncryptedConnection, SALT_LEN, NONCE_LEN, TAG_LEN};
...
pub fn encrypt_oxide_file(
    ...
    let plaintext = bincode::serialize(payload)
        .map_err(|e| OxideFileError::Serialization(e))?;
...
pub fn decrypt_oxide_file(
    ...
    let payload: EncryptedPayload = bincode::deserialize(&plaintext)
        .map_err(|e| OxideFileError::Serialization(e))?;
...
pub fn compute_checksum(connections: &[EncryptedConnection]) -> Result<String, OxideFileError> {
    ...
    for conn in connections {
        let conn_bytes = bincode::serialize(conn)
            .map_err(|e| OxideFileError::Serialization(e))?;
