...
- **GPU 加速渲染** - 基于 xterm.js WebGL 插件，配合 RAF 批处理缓冲，轻松处理海量日志输出
- **智能连接池** - 基于引用计数的 SSH Multiplexing，多终端/SFTP 共享单条物理链路，秒级开启新会话
- **内存高效** - Rust 编写的后端，无 GC 开销，支持微秒级任务调度优化
...
- **智能重连** - 网络波动时自动进入输入锁定模式（Standby），保留现场并尝试无缝恢复
- **拓扑感知** - 自动路由计算，智能选择最优跳板路径 (Auto-Route)
- **Known Hosts** - SSH 主机密钥验证与管理
- **环境隔离** - WebSocket 令牌具备时效窗口和一次性验证，防止本地进程劫持
...
### 🤖 AI 内联助手
- **终端内 AI** - `Ctrl+Shift+I` (Windows) / `Ctrl+I` / `⌘+I` (Mac/Linux) 快速唤起 AI 面板
- **上下文感知** - 自动捕获选中文本或可见缓冲区作为上下文
- **OpenAI 兼容** - 支持 OpenAI、Ollama、DeepSeek、OneAPI 等任意兼容端点
- **流式响应** - 实时显示 AI 生成内容
- **一键操作** - 复制、插入或直接执行生成的命令
- **隐私优先** - API Key 仅存储在系统钥匙串，请求直连 AI 服务，无中间服务器

### 📂 SFTP 文件管理
- **快速搜索** - `Ctrl+Shift+F` (Windows) / `Ctrl+F` / `⌘+F` (Mac/Linux) 快速搜索终端缓冲区
- **双窗格浏览器** - 本地/远程文件并排显示
- **拖拽传输** - 直接拖拽上传下载，支持多文件
- **批量操作** - 多选删除、重命名、新建文件夹
- **智能预览**
...
**SSH 连接层**
- 支持直连和多跳 ProxyJump
- 下层使用 `direct-tcpip` 通道复用物理连接，消除重复握手开销
- 零拷贝传输，内存高效
- 支持直连和多跳 ProxyJump
