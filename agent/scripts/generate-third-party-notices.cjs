...
function buildNotices() {
  ...
  // Use cargo-deny since it's already installed.
  ...
  // Filter out oxideterm-agent to avoid leaking local path
  const thirdPartyCrates = crates.filter(c => c.name !== 'oxideterm-agent');
  ...
  let out = '';
  out += '# Third-Party Notices (OxideTerm Agent)\n\n';
  out += 'This file lists third-party crates used by the OxideTerm Agent binary and their detected licenses (including transitive dependencies).\n';
  ...
  out += '## Notes\n\n';
  out += '- **Multi-license policy**: Where a crate offers multiple licenses (e.g. `MIT OR Apache-2.0`), we always exercise the most permissive option available.\n';
